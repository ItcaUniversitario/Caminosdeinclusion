...
export async function precargarCartas(personajesElegidos, numeroCamino) {
    ...
    try {
        for (let jugador of personajesElegidos) {
            // CORRECCIÓN: Usar el personaje en lugar del id/nombre
           const idBruto = jugador.idPersonaje || jugador.personaje || "paula"; 
            const idPersonaje = idBruto.toLowerCase().trim();
            ...
            console.log(`✅ Mazo listo para ${personajeIdBusqueda}: ${mazoDelJugador.length} cartas.`);
        }
    } catch (error) {
